...
;(() => {
  ...
  const root = el(
    ...
    'position:fixed;right:14px;bottom:14px;z-index:2147483000;padding:7px 10px;border-radius:10px;' +
      ...
      'user-select:none;opacity:.74;transition:opacity .15s,outline-color .4s;touch-action:none',
  )
  ...
  // ---------- 拖动 ----------
  // ⚠ 位置存的是绝对坐标，所以**每次落位都要夹进视口** ——
  //   换屏（竖屏↔横屏）、浏览器放大之后视口变小，不夹的话卡片就飘到窗口外面了 ✗
  function applyPos(p) {
  ...
  function clampPos(x, y) {
    ...
    const vw = Math.max(160, window.innerWidth || 800)
    const vh = Math.max(120, window.innerHeight || 600)
    return {
      x: Math.max(2, Math.min(Math.max(2, vw - w - 2), x)),
      y: Math.max(2, Math.min(Math.max(2, vh - h - 2), y)),
    }
  }
  // 夹过的落位；返回「有没有被夹回来」——被夹了就闪一下，让她知道卡片在哪 ✓
  function place(x, y) {
    const c = clampPos(x, y)
    applyPos(c)
    return c.x !== Math.round(x) || c.y !== Math.round(y)
  }
  function flash() {
    try {
      root.style.outline = '2px solid rgba(143,208,160,.9)'
      setTimeout(() => {
        root.style.outline = '0 solid rgba(143,208,160,0)'
      }, 900)
    } catch (e) {}
  }
  // 回默认位置（右下角）—— 卡片找不见的时候靠它
  function resetPos() {
  ...
  try {
    ...
    if (saved && typeof saved.x === 'number' && typeof saved.y === 'number') {
      requestAnimationFrame(() => {
        if (place(saved.x, saved.y)) flash() // 说明换过屏 / 放大过
      })
    }
  } catch (e) {}
  ...
  root.addEventListener('pointermove', (ev) => {
    ...
    if (drag.moved) place(drag.x + dx, drag.y + dy) // 拖的时候也夹着，拖不出屏幕 ✓
  })
  ...
  // 视口一变就重摆 —— 竖屏横屏来回切、浏览器放大缩小，都靠这个 ✓
  const 重摆 = () => {
    if (!root.style.left) return
    const r = root.getBoundingClientRect()
    place(r.left, r.top)
  }
  window.addEventListener('resize', 重摆)
  window.addEventListener('orientationchange', () => setTimeout(重摆, 300))
  if (window.visualViewport && window.visualViewport.addEventListener) {
    window.visualViewport.addEventListener('resize', 重摆)
  }
  // 右键 = 回到右下角（万一还是找不见）
  ...
  async function renderList(quiet) {
    ...
    if (openList === 'watch') {
      ...
      // 签名里要带上标题 —— 不然她改了会话名字，清单不会跟着变
      const sig = 'w|' + list.map((s) => s.id + ':' + s.title + (watching.has(s.id) ? '1' : '0')).join(',')
  ...
  async function refreshState() {
    ...
    // 扩展的状态分三种：没装 / 装了但页面没开 / 正常。
    // 「来过一次」是永久标记 —— 所以关掉浏览器也不会误报「没装」。
    if (typeof st.everSeen === 'boolean') {
      const nextFresh = st.lastNext > 0 && Date.now() - st.lastNext < 15000
      if (!st.everSeen) {
      ...
      if (!nextFresh) {
        recentEl.textContent = '扩展在，页面没开'
        statusEl.textContent = ''
        const o = el('span', 'color:#8fd0a0;text-decoration:underline;cursor:pointer;font-size:11px', '点我打开 chat.deepseek.com')
        o.dataset.xbClick = '1'
        o.title = '在新标签页打开 DeepSeek'
